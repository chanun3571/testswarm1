...
rospy.Subscriber("/key", String, self.keyCallback, queue_size=1)
...
class STM_Connect():
    ...
    def keyCallback(self, msg):
        ...
        u_x =
        u_y = 
        speed_message = "M0"+"A"+str(int(float(self.wheel[0])*5000))+"B"+str(int(float(self.wheel[1])*5000))+"C"+str(int(float(self.wheel[2])*5000))+"\r\n"
